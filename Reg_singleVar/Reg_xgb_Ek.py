...
max_depth_range = range(3, 100, 10)
...
grid_search = GridSearchCV(estimator=xgb, param_grid=param_grid, cv=5, scoring=scoring, refit='r2', return_train_score=True)
...
print(results_xgb.columns)

results_xgb = results_xgb[['param_n_estimators', 'param_max_depth', 'mean_test_r2', 'mean_test_neg_mean_squared_error']]

results_xgb.columns = ['param_n_estimators', 'param_max_depth', 'mean_test_r2', 'mean_test_neg_mean_squared_error']

results_xgb.loc[:, 'param_n_estimators'] = results_xgb['param_n_estimators'].astype(int)
results_xgb.loc[:, 'param_max_depth'] = results_xgb['param_max_depth'].astype(int)
...
arr_test_heatmap = results_xgb.pivot(index="param_n_estimators", columns="param_max_depth", values="mean_test_r2")
ax_test = sns.heatmap(arr_test_heatmap, cmap = "Blues", vmin=0.9, vmax=1)
print("\naccuracy heatmap:")
plt.savefig("heatmap_xgb_ak0_tilde.png", dpi=2000)
...
# xgb = XGBRegressor(n_estimators=70, max_depth = 28,random_state=0)
