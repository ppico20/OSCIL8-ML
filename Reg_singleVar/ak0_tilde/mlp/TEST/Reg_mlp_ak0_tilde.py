...
def train_model_HPT(activation, optimizer, learning_rate, l2_rate, dropout_rate, X_train, y_train, X_val, y_val):
    ...
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=60, restore_best_weights=True)
    hist = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=600, callbacks=[es], verbose=1)
...
optimizers_map = {'rmsprop': optimizers.RMSprop}
...
model, history, best_params, best_val_evaluate, tuning_results = tune_hyperparameters(activation_functions, optimizers_map, learning_rates, l2_rates, dropout_rates, X_train_scaled, y_train_np, kfold)
