...
################################## Reading csv ##################################
dataset = pd.read_csv("../compiled_data/data_params_ak0_tilde_max.csv")
...
data = dataset[['epsilon','rho_r','mu_r','La_l','Bo_l']]
columns = ['epsilon','rho_r','mu_r','La_l','Bo_l']

# Selecting labels
label = dataset[['max_ak0_tilde']]

label['max_ak0_tilde'] = label['max_ak0_tilde'].apply(lambda x: 1 if x > 1 else 0)
...
rf = RandomForestClassifier(n_estimators=150, max_depth = 28,random_state=0)
...
ax1 = plt.subplot()
shap.summary_plot(rf_shap_values[1], X_test_scaled)
